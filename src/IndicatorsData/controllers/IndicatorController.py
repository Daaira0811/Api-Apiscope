...
class IndicatorController():
    ...
    #que se requiera para realizar el cálculo de c
    def get_Indicators_from_Db(self):
        try:
            indicators=[]
            query="SELECT V.id_indicador, B.nombre FROM indicador_mesa V  INNER JOIN indicador B ON V.id_indicador=B.id_indicador INNER JOIN mesa N ON V.id_mesa=N.id_mesa AND N.nombre='epanueve'".format(config('Tablename'))
...
#Función que retorna busca los indicadores asociados a la mesa y retorna los bloques que estén asociados a las variables de cada indicador
    def get_indicators(self):
        variableController=VariableController()
        listBlocks=[]
        #Se obtiene la lista de bloques con habitantes
        listBlocks.append(variableController.get_variable_from_Db(1))
        #Se obtinene los indicadores y sus variables asociadas
